<x-filament-panels::page>
    @if ($this->session->status !== \App\Models\InventorySession::STATUS_IN_PROGRESS)
        <div class="rounded-lg bg-warning-50 p-4 text-warning-700 dark:bg-warning-500/10 dark:text-warning-400">
            {{ __('app.scanning.session_not_open', ['status' => __("app.session.status.{$this->session->status}")]) }}
        </div>
    @endif
    ...
    @if ($this->session->status === \App\Models\InventorySession::STATUS_IN_PROGRESS)
        ...
        <form wire:submit.prevent="submitKeyboardWedge" class="mb-6 flex gap-2">
            <input
                ...
                placeholder="{{ __('app.scanning.keyboard_wedge_placeholder') }}"
                dir="ltr"
                autofocus
                class="fi-input block w-full rounded-lg border-gray-300 text-lg shadow-sm dark:border-gray-600 dark:bg-gray-700"
            />
            <button type="submit" class="fi-btn rounded-lg bg-primary-600 px-4 py-2 text-white">{{ __('app.scanning.add') }}</button>
        </form>
    @endif
    ...
    @if ($showCreateProductForm)
        <div class="mb-6 rounded-lg border border-warning-300 bg-warning-50 p-4 dark:border-warning-500/30 dark:bg-warning-500/10">
            <p class="mb-3 font-semibold">{{ __('app.scanning.unknown_barcode', ['barcode' => $unknownBarcode]) }}</p>
            <form wire:submit.prevent="createPendingProduct" class="space-y-3">
                <input type="text" wire:model="newProductNameEn" placeholder="{{ __('app.scanning.name_en_placeholder') }}" dir="ltr" class="fi-input block w-full rounded-lg" />
                @error('newProductNameEn') <p class="text-sm text-danger-600">{{ $message }}</p> @enderror

                <input type="text" wire:model="newProductNameAr" placeholder="{{ __('app.scanning.name_ar_placeholder') }}" dir="rtl" class="fi-input block w-full rounded-lg" />
                ...
                <select wire:model="newProductCategoryId" class="fi-input block w-full rounded-lg">
                    <option value="">{{ __('app.scanning.no_category') }}</option>
                ...
                <div class="flex gap-2">
                    <button type="submit" class="fi-btn rounded-lg bg-primary-600 px-4 py-2 text-white">{{ __('app.scanning.save_and_count') }}</button>
                    <button type="button" wire:click="cancelUnknownBarcode" class="fi-btn rounded-lg bg-gray-200 px-4 py-2 dark:bg-gray-700">{{ __('app.scanning.cancel') }}</button>
                </div>
            </form>
        </div>
    @endif
    ...
    {{-- Running list --}}
    <div class="space-y-2">
        <h3 class="text-sm font-semibold text-gray-500">{{ __('app.scanning.counted_so_far', ['count' => count($runningList)]) }}</h3>
        ...
        @forelse ($runningList as $line)
            <div class="flex items-center justify-between rounded-lg border border-gray-200 p-3 dark:border-gray-700" wire:key="line-{{ $line['id'] }}">
                <div>
                    <div class="font-medium">{{ app()->getLocale() === 'ar' ? $line['name_ar'] : $line['name_en'] }}</div>
                    <div class="text-sm text-gray-500">{{ $line['sku'] }} — {{ app()->getLocale() === 'ar' ? $line['name_en'] : $line['name_ar'] }}</div>
                </div>
            ...
            @if ($manualAdjustLineId === $line['id'])
                <div class="rounded-lg border border-primary-300 bg-primary-50 p-3 dark:border-primary-500/30 dark:bg-primary-500/10">
                    <form wire:submit.prevent="saveManualAdjust" class="space-y-2">
                        ...
                        <textarea wire:model="manualAdjustNote" placeholder="{{ __('app.scanning.manual_adjust_note_placeholder') }}" class="fi-input block w-full rounded-lg"></textarea>
                        @error('manualAdjustNote') <p class="text-sm text-danger-600">{{ $message }}</p> @enderror

                        <div class="flex gap-2">
                            <button type="submit" class="fi-btn rounded-lg bg-primary-600 px-4 py-2 text-white">{{ __('app.scanning.save') }}</button>
                            <button type="button" wire:click="cancelManualAdjust" class="fi-btn rounded-lg bg-gray-200 px-4 py-2 dark:bg-gray-700">{{ __('app.scanning.cancel') }}</button>
                        </div>
                    </form>
                </div>
            @endif
        @empty
            <p class="text-gray-500">{{ __('app.scanning.nothing_scanned_yet') }}</p>
        @endforelse
    </div>
